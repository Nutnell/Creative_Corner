...
# Initialize a search tool for the Trend Hunter to get real-time data
...
def get_current_trends(topic: str) -> str:
    ...
    """
    try:
        results = search_tool.run(f"latest trends on {topic}")
        llm = get_llm(temp=0.2)
        prompt = f"""
        Analyze the following search results to identify and summarize the key trends for '{topic}'.
        Extract key insights, statistics, and potential sub-topics for a blog post.

        Search Results:
        {results}

        Respond with a structured report on the trends and suggest a clear, compelling title.
        """
...
def generate_draft(topic: str, trends: str) -> str:
    """
    Generates a blog post draft based on a topic and current trends.
    """
    try:
        llm = get_llm(temp=0.7)
        prompt = f"""
        You are a seasoned blogger. Create a well-structured and engaging blog post draft about '{topic}',
        incorporating the following key trends and insights. The draft should be at least 800 words,
        use a compelling narrative, and include clear headings and subheadings.
